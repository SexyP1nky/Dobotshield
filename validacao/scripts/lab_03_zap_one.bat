...
set "_ZAP_CFG=-config network.connection.timeoutInSecs=10 -config network.connection.defaultUserAgent=DoBotShield-TCC-Validation/1.0 -config scanner.maxScanDurationInMins=3"
...
>> "%LOG%" echo CMD: docker run --memory 3000m --memory-swap 3000m -e DVWA_COOKIE=^<cookie^> %IMG_ZAP% zap-full-scan.py -t %URL% -m 1 -T 2 -c /zap/hooks/zap_rules.tsv -r zap_report.html -J zap_report.json -w zap_report.md -z "!_ZAP_CFG!" --hook=/zap/hooks/zap_hook.py -I  (cookie injetado via Replacer pelo hook)
>> "%LOG%" echo ----------------------------------------------------------------
