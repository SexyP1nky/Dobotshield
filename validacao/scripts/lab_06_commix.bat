...
echo.
echo ============================================================
echo   FERRAMENTA: Commix (Command Injection, level 3, faixa controlada)  --  %DATE% %TIME%
...
set "DVWA_COOKIE="
for /f "delims=" %%C in ('docker run --rm --network %NET% -v "%SCRIPTS_FWD%:/scripts:ro" %IMG_PY% python /scripts/dvwa_login.py --no-setup "http://!IP_DVWA!:80" 2^>"%TMP_LOG_DIR%\dvwa_login.commix.stderr.log"') do set "DVWA_COOKIE=%%C"
...
if defined DVWA_COOKIE (
    ...
    if exist "%TMP_LOG_DIR%\dvwa_login.commix.stderr.log" type "%TMP_LOG_DIR%\dvwa_login.commix.stderr.log"
    exit /b 5
)
