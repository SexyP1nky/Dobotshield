...
O relatório é construído **exclusivamente a partir dos logs** (`lab_results/<app>/<cenario>/`):
...
| DoBotShield | **3** (artefatos) | SQLi `target=127.0.0.1 AND 1=1 --` (sem evidência); Source Code Disclosure `target=../` (sem evidência); Spring4Shell → **evidência = `HTTP/1.1 400 Bad Request`** |
...
- **DoBotShield**: os ataques reais (XSS/Path Traversal/RFI/OS cmdi com evidência) **não
  aparecem** — foram bloqueados. Os 3 "HIGH" do XVWA são **falsos-positivos gerados pelo
  próprio bloqueio**: o DoBotShield responde **HTTP 400** aos payloads, e essa resposta
  diferente confunde as heurísticas diferenciais do ZAP (SQLi booleano, divulgação de
  código) e a regra do Spring4Shell — cuja evidência registrada é literalmente
  `HTTP/1.1 400 Bad Request`. Nenhum tem evidência de execução. No DVWA (scan autenticado)
  esses artefatos não surgiram → **0 HIGH**.
...
> Divergência relevante entre ferramentas: o ZAP marca **DoBotShield = SQLi bloqueado**
> (0 HIGH no DVWA), mas o **SQLMap confirma SQLi booleano** no DoBotShield (ver seção 3).
...
| DVWA | DoBotShield | sqli `id` | **VULNERÁVEL** | `id=1' AND 2880=2880 AND 'Ridm'='Ridm` | 41 req, **502 × 10 + 400 × 2** |
...
| XVWA | DoBotShield | sqli `item` | **VULNERÁVEL** | `item=1 AND 5726=5726` | 34 req, **502 × 25 + 400 × 2** |
...
- **DoBotShield**: **deixa a injeção booleana passar** nas duas apps. Ele responde
  **502/400** a uma parte dos payloads (10–25× 502, 2× 400), o que aumenta o número de
  requisições necessárias (34–41 vs 13–20), mas **não impede** a técnica: o boolean-blind
...
## 6. wrk (carga: 12 threads, 400 conexões, 30s)
...
- **DoBotShield**: ~**100% das respostas são non-2xx** (1,5 milhão), com **9–10 ms** de
  latência e ~50k req/s. Esse número alto **não é vazão de aplicação**: é **rejeição na
...
## 7. Quadro consolidado por cenário
...
| **XSS/Path Trav/RFI/cmdi/SSRF** (ZAP) | 6 HIGH c/ evidência | bloqueia (passa só RFI no XVWA) | bloqueia (3 FP por 400 no XVWA) | bloqueia (passa só RFI no XVWA) |
...
- **Códigos de bloqueio**: ModSecurity e Coraza usam **403**; DoBotShield usa **400/502** —
  e esse 400/502 é o que (a) deixa o SQLi booleano do SQLMap passar e (b) gera falsos-positivos
  diferenciais no ZAP.
